"""
Flying Wing - Mission Profile with L1 Adaptive Control

Demonstrates complete mission using L1 adaptive control:
...
Uses L1 adaptive control to handle:
...
from src.control.adaptive_autopilot import L1AdaptiveAutopilot
import matplotlib.pyplot as plt


def main():
    """Run mission profile with L1 adaptive control."""

    print("=" * 70)
    print("Flying Wing - Mission Profile (L1 Adaptive Control)")
    print("=" * 70)
    print()
    ...
    print()
    print("Using L1 Adaptive Control for robust performance")
    print()
    ...
    # Create L1 ADAPTIVE autopilot
    autopilot = L1AdaptiveAutopilot(
        omega_alt=0.05,        # Slow altitude response
        zeta_alt=0.9,          # Heavy damping
        omega_speed=0.08,      # Moderate speed response
        zeta_speed=0.9,        # Heavy damping
        Gamma=1000.0,          # Adaptation rate
        omega_filter=5.0,      # Filter bandwidth
        mass=mass,
        g=32.174
    )
    ...
    while t < max_time and phase != "complete":
        ...
        theta = state.euler_angles[1]
        alpha = state.alpha
        gamma = theta - alpha
        flight_path_angles.append(gamma)

        # Get controls from L1 adaptive autopilot
        elevon, throttle = autopilot.update(
            ...
            current_alpha=alpha,
            current_flight_path_angle=gamma,
            dt=dt
        )
        ...
        # Get debug info
        debug_info = autopilot.get_debug_info()
        sigma_hats.append(debug_info['sigma_hat'])
        ...
        # Progress
        if step % 200 == 0:
            sigma = debug_info['sigma_hat']
            print(f"  t={t:6.1f}s | Phase: {phase:15s} | Alt: {altitude:7.0f} ft | V: {airspeed:5.1f} ft/s | sigma: {sigma:6.1f}")
    ...
    print(f"Final Adaptive Parameter: sigma = {sigma_hats[-1]:.1f}")
    print()
    ...
    ax.set_title('Flying Wing - L1 Adaptive Control Mission')
    ax.legend()
    ax.grid(True)

    plt.savefig('output/mission_adaptive_3d.png', dpi=150, bbox_inches='tight')
    plt.close()

    # Timeline with adaptation
    ...
    axes[0].set_title('Mission Timeline - L1 Adaptive Control')
    axes[0].grid(True)
    ...
    axes[5].plot(times / 60.0, sigma_hats, 'm-', linewidth=2)
    axes[5].set_ylabel('Sigma (Adaptive Param)')
    axes[5].grid(True)

    axes[6].plot(times / 60.0, np.degrees(flight_path_angles), 'c-', linewidth=2)
    axes[6].set_ylabel('Flight Path Angle (deg)')
    axes[6].set_xlabel('Time (minutes)')
    axes[6].grid(True)

    plt.tight_layout()
    plt.savefig('output/mission_adaptive_timeline.png', dpi=150, bbox_inches='tight')
    plt.close()

    print("  Saved: output/mission_adaptive_3d.png")
    print("  Saved: output/mission_adaptive_timeline.png")
    print()
